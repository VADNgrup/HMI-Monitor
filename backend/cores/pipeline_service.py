...
from cores.pipeline_utils import EntityExtractionNormalizer
...
from utils.llm_client import call_llm_image_to_markdown, call_llm_markdown_to_json, ensure_llm_name
...
class PipelineService:
    ...
    def create_source(self, db: Database, payload: dict) -> str:
        now = now_utc()
        document = {
            "name": payload["name"],
            "host": payload["host"],
            ...
            "mode": payload.get("mode", "v2"),
            "last_polled_at": None,
            "created_at": now,
            "updated_at": now,
        }
        inserted = db.kvm_sources.insert_one(document)
        return self.to_id(inserted.inserted_id)
